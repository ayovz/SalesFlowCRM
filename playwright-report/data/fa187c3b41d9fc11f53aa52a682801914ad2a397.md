...
- Name: lead-detail.spec.ts >> Lead Detail >> shows the company name
- Location: tests\lead-detail.spec.ts:43:7
...
Locator: getByText('Detail Corp')
Expected: visible
Error: strict mode violation: getByText('Detail Corp') resolved to 2 elements:
    1) <p class="text-sm text-on-surface-variant">Detail Corp</p> aka getByRole('paragraph').filter({ hasText: 'Detail Corp' })
    2) <dd class="text-sm text-on-surface mt-0.5">Detail Corp</dd> aka getByRole('definition').filter({ hasText: 'Detail Corp' })

Call log:
  - Expect "toBeVisible" with timeout 5000ms
  - waiting for getByText('Detail Corp')
