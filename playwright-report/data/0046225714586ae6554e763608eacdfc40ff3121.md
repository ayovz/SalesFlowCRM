...
- Name: dashboard.spec.ts >> Dashboard >> renders all 8 stat card labels
...
Expected: visible
Error: strict mode violation: getByText('New') resolved to 3 elements:
    1) <button tabindex="0" type="button" class="inline-flex items-center justify-center rounded-lg font-medium transition-colors↵        bg-primary text-on-primary hover:bg-primary-container h-9  px-4 text-sm gap-2↵        disabled:opacity-50 disabled:cursor-not-allowed ">…</button> aka getByRole('button', { name: 'add New Lead' })
    2) <span class="material-symbols-outlined text-xl text-amber-600">fiber_new</span> aka getByText('fiber_new')
    3) <div class="text-xs text-on-surface-variant mt-0.5">New</div> aka getByText('New', { exact: true })
