...
- Name: analytics.spec.ts >> Analytics & Insights >> renders the "Recent Activity" table
- Location: tests\analytics.spec.ts:52:7
...
Locator: getByText('Lead')
Expected: visible
Error: strict mode violation: getByText('Lead') resolved to 15 elements:
    1) <span class="relative flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-colors↵                text-white/55 hover:text-white hover:bg-white/10">…</span> aka getByRole('link', { name: 'people Leads' })
    2) <div class="text-xs text-on-surface-variant mt-0.5">Total Leads</div> aka getByText('Total Leads')
    3) <h2 class="text-xs font-semibold uppercase tracking-wide text-on-surface-variant">Lead Volume Trend</h2> aka getByRole('heading', { name: 'Lead Volume Trend' })
    4) <th class="py-2 pr-3 text-left text-xs font-semibold uppercase tracking-wide text-on-surface-variant">Leads</th> aka getByRole('columnheader', { name: 'Leads' })
    5) <th class="py-2 pr-4 text-left text-xs font-semibold uppercase tracking-wide text-on-surface-variant">Lead</th> aka getByRole('columnheader', { name: 'Lead', exact: true })
    6) <span class="text-xs font-medium text-on-surface whitespace-nowrap">Lead Created</span> aka getByText('Lead Created').first()
    7) <td class="py-2.5 pr-4 text-on-surface-variant max-w-xs truncate">Lead imported from XLSX</td> aka getByRole('cell', { name: 'Lead imported from XLSX' }).first()
    8) <span class="text-xs font-medium text-on-surface whitespace-nowrap">Lead Created</span> aka getByText('Lead Created').nth(1)
    9) <td class="py-2.5 pr-4 text-on-surface-variant max-w-xs truncate">Lead imported from XLSX</td> aka getByRole('cell', { name: 'Lead imported from XLSX' }).nth(1)
    10) <span class="text-xs font-medium text-on-surface whitespace-nowrap">Lead Created</span> aka getByText('Lead Created').nth(2)
    ...

Call log:
  - Expect "toBeVisible" with timeout 5000ms
  - waiting for getByText('Lead')
        ...
        - generic [ref=e46]:
          ...
          - generic [ref=e259]:
            ...
            - table [ref=e265]:
              ...
              - rowgroup [ref=e273]:
                - row "add_circle Lead Created Alex Chen Lead imported from XLSX admin@example.com 6h ago" [ref=e274]:
                  ...
                  - cell "6h ago" [ref=e283]
                - row "add_circle Lead Created Sarah Kim Lead imported from XLSX admin@example.com 6h ago" [ref=e284]:
                  ...
                  - cell "6h ago" [ref=e293]
                - row "add_circle Lead Created James Park Lead imported from XLSX admin@example.com 6h ago" [ref=e294]:
                  ...
                  - cell "6h ago" [ref=e303]
                - row "add_circle Lead Created Maria Lopez Lead imported from XLSX admin@example.com 6h ago" [ref=e304]:
                  ...
                  - cell "6h ago" [ref=e313]
                - row "add_circle Lead Created David Osei Lead imported from XLSX admin@example.com 6h ago" [ref=e314]:
...
```ts
  ...
  7   |     // Wait for summary stat cards to load
  8   |     await page.waitForSelector('text=Total Leads', { timeout: 10_000 })
  9   |   })
  10  | 
  11  |   test('shows "Analytics" as the page title', async ({ page }) => {
  12  |     await expect(page.locator('header h1')).toHaveText('Analytics')
  13  |   })
  14  | 
