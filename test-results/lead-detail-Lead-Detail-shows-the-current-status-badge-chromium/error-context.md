...
- Name: lead-detail.spec.ts >> Lead Detail >> shows the current status badge
- Location: tests\lead-detail.spec.ts:62:7
...
Locator: getByText('Contacted')
Expected: visible
Error: strict mode violation: getByText('Contacted') resolved to 2 elements:
    1) <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium bg-sky-100     text-sky-700">Contacted</span> aka locator('span').filter({ hasText: 'Contacted' })
    2) <option value="Contacted">Contacted</option> aka getByRole('combobox')

Call log:
  - Expect "toBeVisible" with timeout 5000ms
  - waiting for getByText('Contacted')
